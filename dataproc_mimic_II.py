...
# with codecs.open('%s/MIMIC_FILTERED_DSUMS' % MIMIC_2_DIR, 'r', encoding='latin-1') as f:
#     with open('%s/proc_dsums_sentsplit.csv' % MIMIC_2_DIR, 'w') as of:
#         r = csv.reader(f, delimiter='|')
#         #header
#         next(r)
#         w = csv.writer(of)
...
#                     test_w.writerow(row)
#
# with open('%s/proc_dsums_sentsplit.csv' % MIMIC_2_DIR, 'r') as nf:
#     with open('%s/test_dsums_sentsplit.csv' % MIMIC_2_DIR, 'w') as test_f:
#         with open('%s/train_dsums_sentsplit.csv' % MIMIC_2_DIR, 'w') as train_f:
#             r = csv.reader(nf, delimiter=',')
#             test_w = csv.writer(test_f)
#             train_w = csv.writer(train_f)
#             #header
...
# df = pd.read_csv('%s/train_dsums_sentsplit.csv' % MIMIC_2_DIR)
...
#
# df = pd.read_csv('%s/test_dsums_sentsplit.csv' % MIMIC_2_DIR)
# df['length'] = df.apply(lambda row: len(row[2].split()) if not pd.isnull(row[2]) else 0, axis=1)
# df = df[df['length'] > 1]
# df = df.sort_values(['length'])
...
# fasttext_file = word_embeddings.fasttext_embeddings('full', '%s/proc_dsums.csv' % MIMIC_2_DIR, 100, 3, 5)

# extract_wvs.gensim_to_embeddings('%s/processed_full.w2v' % MIMIC_2_DIR, '%s/vocab.csv' % MIMIC_2_DIR, None)
# extract_wvs.gensim_to_fasttext_embeddings('%s/processed_full.fasttext' % MIMIC_2_DIR, '%s/vocab.csv' % MIMIC_2_DIR, None)